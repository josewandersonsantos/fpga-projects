...
module main(clk_in, rst_in);
    ...
    pc uut_pc(.clk(clk), .rst(rst), .en(pc_en), .inc(pc_inc), .out(pc_out));
    memory uut_mem(.clk(clk), .rst(rst), .en(mem_en), .ld(mem_ld), .bus(bus), .out(mem_out));
    ...
    ir uut_ir(.clk(clk), .rst(rst), .en(ir_en), .ld(ir_ld), .bus(bus), .out(ir_out));
